...
export default function Home() {
  const generateQuery = useQuery(["generate"], async () => {
    const text = await fetch(
      " https://calm-disk-0cc0.ruchiket100.workers.dev/generate",
    )
    return text.json()
  })
  ...
  useEffect(() => {
    const isInputRefFocused = () => {
      if (document) return document.activeElement === inputRef.current || false
      return false
    }
          ...
          {!generateQuery.isLoading && (
            <>
              ...
              <textarea
                ...
                ref={inputRef}
                value={input}
                className=' z-[999] absolute bg-transparent input-transparent outline-none opacity-0  w-full h-full'
                onChange={handleInputChange}
              />
              <SyntaxHighlighter
                customStyle={{ ...styles, zIndex: "30" } as any}
                language='typescript'
                style={srcery}
              >
                {input}
              ...
              {/* Reference Layer */}
              <SyntaxHighlighter
                customStyle={{ backgroundColor: "transparent", color: "gray" }}
                language='typescript'
                style={ascetic}
              >
                {text || ""}
